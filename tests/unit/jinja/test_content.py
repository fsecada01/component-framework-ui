...
def test_card_renders_default_slot(render):
    html = render("Card.jinja", content="Body text", header="", footer="", **{"class": ""})
    assert "Body text" in html
    assert "card" in html


def test_card_renders_header_when_provided(render):
    html = render("Card.jinja", content="Body", header="My Title", footer="", **{"class": ""})
    assert "My Title" in html
    assert "card-header" in html


def test_card_omits_header_when_empty(render):
    html = render("Card.jinja", content="Body", header="", footer="", **{"class": ""})
    assert "card-header" not in html


def test_card_renders_footer_when_provided(render):
    html = render("Card.jinja", content="Body", header="", footer="Footer text", **{"class": ""})
...
def test_table_renders_headers(render):
    ...
    html = render("Table.jinja", columns=columns, rows=[], hx_target="", hx_url="", **{"class": ""})
    assert "Name" in html
    assert "Age" in html
    assert "<th>" in html
...
def test_table_renders_rows(render):
    ...
    html = render("Table.jinja", columns=columns, rows=rows, hx_target="", hx_url="", **{"class": ""})
